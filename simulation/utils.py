...
def fetch_force_coefficients(path):
    ...
    names = ["t", "Cd", "Cd_f", "Cd_r", "Cl", "Cl_f", "Cl_r", "CmPitch", "CmRoll",
             "CmYaw", "Cs", "Cs_f", "Cs_r" ]
    df_list = []
    for tf in times:
...
def fetch_surface_data(path, remove_te=False, symmetric=True):
    ...
    """
    d = pd.read_csv(path, sep=" ", header=None, skiprows=[
                           0, 1])
    if (d.shape[1] > 4):
        data = pd.read_csv(path, sep=" ", skiprows=[
                           0, 1], header=None, names=["x", "y", "z", "tau_x", "tau_y", "tau_z"])
    ...
    if symmetric:
        ...
        z85 = data.z[(data.x > 0.83*x_max) & (data.x < 0.87*x_max)].mean()
        chamber = np.zeros(len(data))
        x85 = 0.85 * x_max
        chamber = data.x.values * z85 / x85
        chamber[data.x > x85] = z85 * (1.0 - (data.x[data.x > x85].values - x85) / (x_max - x85))
    limit = 0.999 if remove_te else 1.1
    x_up = data[(data.z >= chamber) & (data.x < limit*x_max)].x
    x_up = x_up.values / x_max
    y_up = data[(data.z >= chamber) & (data.x < limit*x_max)].y
    y_up = y_up.values / x_max
    f_up = data[(data.z >= chamber) & (data.x < limit*x_max)].f.values
    x_low = data[(data.z < chamber) & (data.x < limit*x_max)].x
    x_low = x_low.values / x_max
    y_low = data[(data.z < chamber) & (data.x < limit*x_max)].y
    y_low = y_low.values / x_max
    f_low = data[(data.z < chamber) & (data.x < limit*x_max)].f.values
    ind_up = np.argsort(x_up)
    ind_low = np.argsort(x_low)
    return x_up[ind_up], y_up[ind_up], f_up[ind_up], x_low[ind_low], y_low[ind_low], f_low[ind_low]
